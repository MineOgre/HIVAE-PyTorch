...
def read_data(data_file, types_file, miss_file, true_miss_file):
    ...
    for i in range(np.shape(data)[1]):
        types_dict[i]['dim'] = int(types_dict[i]['dim'])
...
def samples_concatenation_xz(samples):
    for i, batch in enumerate(samples):
        if i == 0:
            # samples_x = np.concatenate(batch['x'], 1)
            samples_x = torch.cat(batch['x'], 1)
            samples_z = batch['z']
        else:
            # samples_x = np.concatenate([samples_x, np.concatenate(batch['x'], 1)], 0)
            # samples_z = np.concatenate([samples_z, batch['z']], 0)
            samples_x = torch.cat([samples_x, torch.cat(batch['x'], 1)], 0)
            samples_z = torch.cat([samples_z, batch['z']], 0)

    return samples_z, samples_x

def p_params_concatenation(parameter_list, types_dict):
        ...
        if i == 0:
            ...
            for v in range(len(types_dict)):
                if types_dict[v]['type'] == 'pos' or types_dict[v]['type'] == 'real':
                    out_list[v][0] = torch.cat((out_list[v][0],batch[v][0]))
                    out_list[v][1] = torch.cat((out_list[v][1],batch[v][1]))
                else:
                    ##TODO: Burası niye 0 dimden bağlanıyor?
                    out_list[v] = torch.cat([out_list[v], batch[v]], 0)

    return out_list

def p_params_x_concatenation(parameter_list, types_dict):
    data = []
    for i, batch in enumerate(parameter_list):
        values = []
        for j, typ in enumerate(types_dict):
            if typ['type'] == 'real':
                values.append(batch['x'][j][0])
            elif typ['type'] == 'pos':
                values.append(np.exp(batch['x'][j][0]))
            elif typ['type'] == 'cat':
                values.append(np.argmax(batch['x'][j], axis=1).reshape((-1,1)))
            else:
                values.append([])
        data.append(np.concatenate(values, 1))
...
def discrete_variables_transformation(data, types_dict):
    ...
    for d in range(len(types_dict)):
        ...
        if types_dict[d]['type'] == 'cat':
            output.append(torch.reshape(torch.argmax(data[:, ind_ini:ind_end], 1), [-1, 1]).to(torch.float64))
        elif types_dict[d]['type'] == 'ordinal':
            output.append(torch.reshape(torch.sum(data[:, ind_ini:ind_end], 1) - 1, [-1, 1]).to(torch.float64))
        else:
...
def statistics(loglik_params, types_dict):
    ...
    for d, attrib in enumerate(loglik_params):
        ...
        # Only for log-normal
            ...
            # loglik_mean.append(np.array(np.maximum(np.exp(attrib[0] + 0.5 * attrib[1]) - 1.0, 0.0)))
            # loglik_mode.append(np.array(np.maximum(np.exp(attrib[0] - attrib[1]) - 1.0, 0.0)))
            mean = torch.exp(attrib[0] + 0.5 * attrib[1]) - 1.0
            mean[mean < 0] = 0
            loglik_mean.append(mean)
            mode = torch.exp(attrib[0] - attrib[1]) - 1.0
            mode[mode < 0] = 0
            ...
            # loglik_mean.append(np.reshape(np.array(np.argmax(attrib, 1)).astype(float), [-1, 1]))
            # loglik_mode.append(np.reshape(np.array(np.argmax(attrib, 1)).astype(float), [-1, 1]))
            loglik_mean.append(torch.reshape(torch.argmax(attrib, 1).to(torch.float64), [-1, 1]))
            loglik_mode.append(torch.reshape(torch.argmax(attrib, 1).to(torch.float64), [-1, 1]))

    # return np.transpose(np.squeeze(loglik_mean)), np.transpose(np.squeeze(loglik_mode))
    return torch.cat(loglik_mean, 1), torch.cat(loglik_mode, 1)
...
def error_computation(x_train, x_hat, types_dict, miss_mask):
    ...
    for dd in range(len(types_dict)):
        ...
        if types_dict[dd]['type'] == 'cat':
            ind_end = ind_ini + 1
            error_observed.append(torch.mean((x_train[miss_mask[:, dd] == 1, ind_ini:ind_end] !=
                                              x_hat[miss_mask[:, dd] == 1, ind_ini:ind_end]).float()))
            if torch.sum((miss_mask[:, dd] == 0).float(), 0) == 0:
                error_missing.append(0)
            else:
                error_missing.append(torch.mean(
                    (x_train[miss_mask[:, dd] == 0, ind_ini:ind_end] != x_hat[miss_mask[:, dd] == 0, ind_ini:ind_end]).float()))
        # Mean "shift" error
        elif types_dict[dd]['type'] == 'ordinal':
            ind_end = ind_ini + 1
            error_observed.append(torch.mean(np.abs(
                x_train[miss_mask[:, dd] == 1, ind_ini:ind_end] - x_hat[miss_mask[:, dd] == 1, ind_ini:ind_end])) / int(
                types_dict[dd]['dim']))
            if torch.sum((miss_mask[:, dd] == 0).float(), 0) == 0:
                error_missing.append(0)
            else:
                error_missing.append(torch.mean(np.abs(
                    x_train[miss_mask[:, dd] == 0, ind_ini:ind_end] - x_hat[miss_mask[:, dd] == 0,
                                                                      ind_ini:ind_end])) / int(types_dict[dd]['dim']))
        # Normalized root mean square error
        else:
            ind_end = ind_ini + int(types_dict[dd]['dim'])
            norm_term = torch.max(x_train[:, dd]) - torch.min(x_train[:, dd])
            error_observed.append(torch.sqrt(torch.nn.MSELoss()(x_train[miss_mask[:, dd] == 1, ind_ini:ind_end],
                                                             x_hat[miss_mask[:, dd] == 1,
                                                             ind_ini:ind_end])) / norm_term)

            if torch.sum(miss_mask[:, dd] == 0, 0) == 0:
                error_missing.append(0)
            else:
                error_missing.append(torch.sqrt(torch.nn.MSELoss()(x_train[miss_mask[:, dd] == 0, ind_ini:ind_end],
